...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            use_pbr = get_pbr_choice("Was PBR enhancement used during encryption? (y/n, default=y): ")
            block_size = 8
            if use_pbr:
                block_size = get_block_size_input("Enter block size used during encryption (default=8): ")
            
            plaintext, err = decrypt_text(token, pwd, rounds, use_pbr, block_size)
            if err:
                print("\nError:", err)
            else:
                print("\nDecrypted plaintext:\n")
            ...
            encrypted_bytes = pbr_encrypt_bytes(message_bytes, keyword, block_size)
            ...
            # Decrypt
            decrypted_bytes = pbr_decrypt_bytes(encrypted_bytes, keyword, block_size)
            decrypted_message = decrypted_bytes.decode('utf-8')
            
            print(f"\n--- PBR Decryption ---")
            print(f"Decrypted Message:     {decrypted_message}")
            print(f"Match Original:        {'Yes' if message == decrypted_message else 'No'}")
